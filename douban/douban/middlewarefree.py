...
class IPProxy(object):
    PROXY_URL = "http://www.zdopen.com/ShortProxy/GetIP/?api=202004160915251131&akey=924daef386dbb305&count=1&order=2&type=3"
    def __init__(self):
    ...
    def process_response(self, request, response, spider):
        contents = eval(response.text).get("msg")
        print(contents)
        if contents:
    ...
    def update_proxy(self):
        self.lock.acquire()
        if self.current_proxy is None or self.current_proxy.is_block:
            response_json = requests.get(self.PROXY_URL).json()
            try:
                print(response_json)
                self.current_proxy = ProxyModel(response_json['data']['proxy_list'][0])
